...
BOOL CFlashWnd::OnAfterShowingContent()
{
	...
	hr = m_lpControl->LoadMovie(0, szFile);
	if (FAILED(hr))
		return FALSE;
	hr = m_lpControl->Play();
	if (FAILED(hr))
...
void CFlashWnd::SetQuality(UINT value)
{
	
	switch(value)
	{
	case ID_RIGHTMENU_LOW:
		m_lpControl->put_Quality2(::SysAllocString(L"Low"));
		break;
	case ID_RIGHTMENU_MID:
		m_lpControl->put_Quality2(::SysAllocString(L"Medium"));
		break;
	case ID_RIGHTMENU_HIGH:
		m_lpControl->put_Quality2(::SysAllocString(L"High"));
		break;
	default:
		break;
	}
